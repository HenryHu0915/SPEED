...
const handleApprove = async (index: number) => {
  ...
  try {
    ...
    const response = await axios.post(
      `https://speed-1-backend-sigma.vercel.app/articles/approveArticle?_id=${article.id}`, // Use '_id' field in the URL
      { "approved": true }
    );
...
const handleReject = async (index: number) => {
  ...
  try {
    ...
    const response = await axios.post(
      `https://speed-1-backend-sigma.vercel.app/articles/rejectArticle?_id=${article.id}`, // Use '_id' field in the URL
      { "rejected": true }
    );
    // Log the response for debugging
